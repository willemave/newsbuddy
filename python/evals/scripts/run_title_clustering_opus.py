...
from newsly_evals.artifacts import (
    TITLE_CLUSTERING_TITLE_ROW_ARTIFACT,
    read_jsonl_records,
...
def _build_user_prompt(*, batch_id: str, rows: list[dict[str, Any]]) -> str:
    return render_user_prompt(batch_id=batch_id, rows=rows)
...
def _prepare_batches(
    *,
    ...
    rows = read_jsonl_records(input_path)
    title_rows = [title_only_row(row) for row in rows]
    ...
    title_rows_sha256 = write_jsonl_artifact(
        title_rows_path,
        sorted_rows,
        artifact_type=TITLE_CLUSTERING_TITLE_ROW_ARTIFACT,
    )
    ...
    manifest = {
        "schema_version": 1,
        "artifact_type": "newsly.title_clustering.opus_manifest",
        "input_jsonl": str(input_path),
        "title_only_rows": str(title_rows_path),
        "title_only_rows_sha256": title_rows_sha256,
