...
#[test]
fn scheduled_missing_frontier_and_oversized_head_stay_bounded() {
    let mut target = podcast_target();
    target.limit = 2;
    target
        .known_urls
        .insert("https://example.com/removed.mp3".to_owned());
    let document = br#"<rss version="2.0"><channel><title>Undated reordered feed</title>
    <item><guid>stable-id</guid><enclosure url="https://example.com/changed.mp3" type="audio/mpeg"/></item>
    <item><guid>b</guid><enclosure url="https://example.com/b.mp3" type="audio/mpeg"/></item>
    <item><guid>c</guid><enclosure url="https://example.com/c.mp3" type="audio/mpeg"/></item>
    </channel></rss>"#;
    let first = normalize_feed_document(&target, document).unwrap();
    assert_eq!(first.items.len(), 2);
    for item in first.items {
        if let ScrapedItem::Content(item) = item {
            target.known_urls.insert(item.url);
        }
    }
    assert!(
        normalize_feed_document(&target, document)
            .unwrap()
            .items
            .is_empty()
    );
    target.entry_selection = FeedEntrySelection::SkipKnown;
    let explicit = normalize_feed_document(&target, document).unwrap();
    assert_eq!(
        explicit.items.len(),
        1,
        "remaining history requires explicit catch-up"
    );
}
