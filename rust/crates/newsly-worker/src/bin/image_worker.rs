...
use newsly_worker::config::{ImageWorkerProcessConfig, WorkerLogFormat};
...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        ...
        "Newsly Rust image worker started; runtime ownership must be explicitly cut over before rows are claimable"
    );
