...
use newsly_worker::config::{DiscussionWorkerProcessConfig, WorkerLogFormat};
...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        ...
        "Newsly Rust discussion worker started; runtime ownership must be explicitly cut over before rows are claimable"
    );
