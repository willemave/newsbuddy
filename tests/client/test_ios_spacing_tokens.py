...
def test_swiftui_views_use_sensory_feedback_for_haptics() -> None:
    manual_haptic = re.compile(
        r"UI(?:Impact|Notification|Selection)FeedbackGenerator"
    )
    offenders: list[str] = []
...
def test_knowledge_library_sheets_use_single_model_destination() -> None:
    ...
    learning_deck_sheet_source = (
        VIEWS_ROOT / "Components/LearningDeckListSheet.swift"
    ).read_text()
