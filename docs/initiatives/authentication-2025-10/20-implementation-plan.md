...
**Files:**
- Create: `migrations/alembic/versions/YYYYMMDD_create_users_table.py` (autogenerated)

**Step 1: Generate migration**

```bash
source .venv/bin/activate && alembic -c migrations/alembic.ini revision --autogenerate -m "create users table"
```

Expected: Creates new migration file in `migrations/alembic/versions/`

**Step 2: Review the generated migration**

```bash
ls -la migrations/alembic/versions/ | tail -1
```
...
```bash
source .venv/bin/activate && alembic -c migrations/alembic.ini upgrade head
```
...
```bash
git add migrations/alembic/versions/*create_users_table.py
...
**Files:**
- Create: `migrations/alembic/versions/YYYYMMDD_migrate_to_user_based_tracking.py`

**Step 1: Create manual migration file**

```bash
source .venv/bin/activate && alembic -c migrations/alembic.ini revision -m "migrate to user based tracking"
```

**Step 2: Write migration**

Edit the newly created migration file in `migrations/alembic/versions/` and replace with:
...
```bash
git add migrations/alembic/versions/*migrate_to_user_based_tracking.py
