...
- `internal/api` is checked-in generated Go model/enum code from `app.models.contracts_registry`, plus hand-written query parameter helpers.
- `internal/runtime` is the hand-written HTTP client layer for generated API models, polling/wait behavior, QR auth, library sync, and source subscription.
...
| `internal/runtime/` | Hand-written HTTP client wrappers, polling/wait behavior, QR link/library/source custom calls. |
| `internal/api/` | Generated Go API models/enums plus small CLI query parameter helpers. |
...
- Regenerate the CLI OpenAPI derivative and Go API models with `./scripts/generate_agent_cli_artifacts.sh`.
- Go model generation flows through `scripts/generate_go_contracts.py` and `scripts/contracts_codegen/go_emitter.py`; the runtime client is hand-written.
