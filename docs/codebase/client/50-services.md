...
- Voice capture is dictation/transcription-oriented: `SpeechTranscribing` reserves one exclusive `SpeechTranscriptionSession` before microphone startup awaits, and the session exposes state, transcript, error, and stop-reason updates as an `AsyncStream`. Each surface passes `SpeechRecordingDeadlines`; the standard policy is a 10-second no-speech timeout and 60-second absolute cap, while onboarding explicitly uses a 30-second cap. `VoiceDictationService` owns live audio, deadline enforcement, background/interruption cleanup, and backend transcription; `E2EScriptedSpeechTranscriber` deterministically drives success, empty, failure, silence, no-speech, maximum-duration, and background-cancel paths through the same contract; mic haptics are SwiftUI `.sensoryFeedback` on `TapToTalkMicButton`.
...
| Content/news/source APIs | `ContentService.swift`, `ScraperConfigService.swift`, `BadgeStatsStore.swift` | Content/news lists/details/actions, source config, and canonical badge counts. |
...
| Onboarding | `OnboardingService.swift` | Audio discovery, discovery-status polling, completion, and tutorial-state API calls. |
...
| Voice/transcription | `SpeechTranscribing.swift`, `VoiceDictationService.swift`, `OpenAIService.swift` | Exclusive dictation sessions, deterministic E2E scenarios, audio upload transcription, deadlines, and microphone capture. |
