...
- Non-briefing view models and app stores use Swift Observation (`@Observable`) and are owned with `@State` in SwiftUI; Briefing remains on the legacy `ObservableObject` path for the separate briefing initiative.
...
- Feature view models handle Knowledge, onboarding, custom narrations, Learning Decks, source settings, search, submissions, and tab coordination.
...
- ChatSessionViewModel and TweetSuggestionsViewModel receive auth, token, and transcription availability dependencies instead of reading auth/OpenAI/app settings singletons directly.
...
| Chat | `ChatSessionViewModel.swift`, `ChatSessionsViewModel.swift`, `ChatTimelineReconciler.swift`, `VoiceDictationCoordinator.swift` | Chat session/message lifecycle, render timeline reconciliation, and shared dictation event handling. |
| Knowledge/search/discovery | `KnowledgeHubViewModel.swift`, `SearchViewModel.swift`, `DiscoveryPersonalizeViewModel.swift` | Knowledge hub, search, and discovery-personalization state. |
