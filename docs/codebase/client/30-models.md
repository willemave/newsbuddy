...
| `client/newsly/newsly/Models/ChatSessionDetail.swift` | `struct ChatSessionDetail`, `struct CreateChatSessionResponse`, `struct SendChatMessageResponse`, `enum CodingKeys`, `struct StartDigDeeperChatResponse`, `enum CodingKeys`, `struct MessageStatusResponse`, `enum CodingKeys`, `struct InitialSuggestionsResponse`, `enum CodingKeys`, +3 more | Full chat session details with message history |
...
| `client/newsly/newsly/Models/NewsMetadata.swift` | `struct NewsMetadata`, `struct NewsArticle`, `struct NewsAggregator`, `struct TopComment` | Short-form news item metadata used by the news list and detail surfaces. |
