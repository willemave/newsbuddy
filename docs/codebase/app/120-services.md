...
| `app/services/daily_digest_chat.py` | `build_daily_digest_context_snapshot`, `start_daily_digest_chat` | Helpers for starting daily-digest dig-deeper chats. |
| `app/services/daily_news_digest.py` | `DailyDigestSourceItem`, `DailyDigestUpsertResult`, `normalize_timezone`, `get_local_day_utc_bounds`, `resolve_target_local_date_for_generation`, `collect_daily_news_sources`, `digest_requires_regeneration`, `upsert_daily_news_digest_for_user_day`, `enqueue_daily_news_digest_task` | Daily per-user news digest synthesis and enqueue helpers. |
