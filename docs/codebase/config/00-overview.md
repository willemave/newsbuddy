...
## Purpose
File-backed feed, scraper, aggregator, YouTube, and tooling configuration used by scraper bootstrapping, import scripts, and guardrail tooling.
...
| `config/substack.yml` | Curated Substack feeds | File-backed/import seed data; not used by onboarding source discovery. Live user subscriptions are DB-backed. |
| `config/substack.example.yml` | Example Substack feed file | Template only. |
| `config/atom.yml` | Curated Atom/RSS feed defaults | File-backed/import seed data; not used by onboarding source discovery. |
| `config/atom.example.yml` | Example Atom feed file | Template only. |
| `config/podcasts.yml` | Podcast RSS feed inputs | Import/default feed data used by scripts and podcast scraper inputs, not onboarding source discovery. |
