...
A14. Onboarding guidance never obscures or intercepts actionable controls. The welcome presents the complete interactive ring and Buddy until Continue is tapped, then transitions to the small upper-left guide for the remaining flow. Dragging turns the sculpture with bounded displacement; releasing adds damped momentum and returns it toward center. Advancing never waits on animation. Resumed flows start with the small guide, and loading states use the centered Buddy indicator. Reduced Motion presents a settled static logo without ambient, inertial, or docking motion.

A15. The signed-out landing presents the complete slate ring and Buddy as an interactive matte 3D sculpture, on transparency so it sits on the surface rather than in a field of its own. Sign-in stays immediately available; interaction is confined to the logo stage. Reduced Motion uses the settled static brand mark. Decorative motion stops while the app is inactive and is released when the screen leaves. The icon's own square field is reserved for places that are showing the app icon as an icon. The standalone Buddy is reserved for in-flow guidance and loading feedback.
