...
for i, (slug, name, dom, acc, ground) in enumerate(RECOLOR_PALETTES, start=1):
    _add(
        ...
        "is unchanged.",
        STYLE_BASE + f"Recolor only: make the brushed ring {dom}, the open book {acc}, and the "
...
for i, (slug, name, dom, acc, ground) in enumerate(RECOLOR_PALETTES, start=9):
    _add(
        ...
        STYLE_BASE + f"Recolor only: make the body {dom}, the round spectacles {acc}, and the "
...
for i, (slug, name, dom, acc) in enumerate(TWOTONE_PALETTES, start=17):
    _add(
        ...
        STYLE_BASE + "Convert this to a strict TWO-TONE mark: exactly two flat colors and "
