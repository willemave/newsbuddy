...
- **Decisions:** Treat the accepted durable task as the refresh completion authority instead of restoring the stale synchronous no-work preflight. Every accepted refresh returns a required task id; task completion is terminal even when the Briefing version is unchanged. Manual refresh uses the existing full-rebuild transaction so read sources disappear only when an unread replacement edition publishes successfully. Prefer structured nested Note Tweet entities, with a bounded exact-`t.co` redirect fallback only when the note contains one short link and no expanded external URL.
- **Changes:** Added an owner-scoped refresh-status endpoint and generated client contracts, task-aware iOS polling with same-version and failure handling, and the reduced-motion-aware animated AppMark waiting state. Manual refresh now requests a deduplicated full rebuild, which reseeds only eligible unread sources and atomically compacts the prior edition. X mapping now reads nested note entity sets and the content worker can resolve a lone trusted short link before falling back to native tweet text. Also retained the recovered Briefing source-link styling commit on this branch.
- **Validation:** Rust formatting and warning-denied Clippy passed for API, contracts, DB, providers, and worker. Focused provider tests passed (5), worker content-handler tests passed (9), and the owner-scoped refresh observation and read-article full-rebuild tests passed against local PostgreSQL. Public contracts regenerated and drift check passed. All 11 `BriefingViewModelRefreshTests` passed on an Xcode-built iPhone 17 Simulator, including same-version completion and failed-task coverage. Diff checks passed.
...
- **Changes:** Merged `origin/main` into the local `main` release candidate with no implementation conflicts; the only manual resolution retained both sides of `docs/log.md`. Made the AXe smoke explicitly return to Briefing when a prior run persisted the Knowledge tab for the same disposable user ID. Pointed the smoke API and workers at the shared `/data/content_bodies` mount so a worker-published Learning Deck is readable from the API container instead of being stranded in a container-private relative path.
- **Validation:** Merge-tree inspection and `git diff --check` passed. The deterministic Rust, SQLx, contract, Python, native iOS, and AXe phases passed repeatedly. Live smoke exposed and isolated a stale shared E2B template, the persisted-tab AXe harness bug, and the container-private artifact root; direct bootstrap capability probing confirmed the temporary exact-candidate template has Playwright and Chromium. The complete corrected gate, exact-SHA push, Docker deployment, and production health proof remain in progress.
