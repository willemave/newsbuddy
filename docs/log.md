...
- **Decisions:** Treat the accepted durable task as the refresh completion authority instead of restoring the stale synchronous no-work preflight. Every accepted refresh returns a required task id; task completion is terminal even when the Briefing version is unchanged. Prefer structured nested Note Tweet entities, with a bounded exact-`t.co` redirect fallback only when the note contains one short link and no expanded external URL.
- **Changes:** Added an owner-scoped refresh-status endpoint and generated client contracts, task-aware iOS polling with same-version and failure handling, and the reduced-motion-aware animated AppMark waiting state. X mapping now reads nested note entity sets and the content worker can resolve a lone trusted short link before falling back to native tweet text. Also retained the recovered Briefing source-link styling commit on this branch.
- **Validation:** Rust formatting and warning-denied Clippy passed for API, contracts, DB, providers, and worker. Focused provider tests passed (5), worker content-handler tests passed (9), and the owner-scoped PostgreSQL refresh observation test passed against local PostgreSQL. Public contracts regenerated and drift check passed. All 11 `BriefingViewModelRefreshTests` passed on an Xcode-built iPhone 17 Simulator, including same-version completion and failed-task coverage. Diff checks passed.
